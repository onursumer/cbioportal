...
package org.cytoscapeweb.model {
    ...
    import org.cytoscapeweb.model.converters.CbioSBGNMLConverter;
    ...
    import org.cytoscapeweb.model.converters.SBGNMLConverter;
    ...
    public class GraphProxy extends Proxy {
        ...
        public function loadGraph(network:*, layout:*, genomicData:*):void {
            try {
                var ds:DataSet;
                ...
                if (network is String)
				{
                    ...
                    if (xml != null && xml.name() != null) {
						...
						var isSbgnml: Boolean = xml.name().localName === CbioSBGNMLConverter.SBGNML;
                        
                        if (isGraphml) {
                            // GraphML:
                            ds = new GraphMLConverter().parse(xml, genomicData);
                        } 						
						else if(isSbgnml)
						{
							// define the regex pattern to remove the namespaces from the string
							var pattern:RegExp = new RegExp("xmlns[^\"]*\"[^\"]*\"", "");
							
							// remove the namespaces from the string representation of the XML
							network = network.replace(pattern, "");
							
							// remove namespace
							xml = new XML(network);
							
							ds = new CbioSBGNMLConverter().parse(xml, genomicData);
						}
						else {
                            // XGMML:
